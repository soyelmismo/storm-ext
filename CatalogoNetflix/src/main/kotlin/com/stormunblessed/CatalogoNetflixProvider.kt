 ...
 * Mexico/LATAM in Spanish. Resolves streams seamlessly across installed providers
 * (Pelisplus4K, PoseidonHD, Cinecalidad, TvClub, HDFull, etc.).
 */
class CatalogoNetflixProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        // Cross-provider search across other installed providers
        val matchName = filterName(title)
        val searchYear = year
        var matches = findMatches(title, matchName, searchYear, data.isTv)
        if (matches.isEmpty()) {
            val altTitle = details.originalTitle?.takeIf { it.isNotBlank() && it != title }
                ?: details.originalName?.takeIf { it.isNotBlank() && it != title }
                ?: details.englishTitle?.takeIf { it.isNotBlank() && it != title }
            if (altTitle != null) {
                matches = findMatches(altTitle, filterName(altTitle), searchYear, data.isTv)
            }
        }
